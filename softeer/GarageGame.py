...
'''
모든 경우별로 다 해보기. 위에 새로 오는애들마다 결과 다를것이기 때문.
...
가로로 놓고, 뭐든 선택 가능 : 15*15

점수는 직사각형, 사라지는 숫자는 같은 색상만.

색상 번호는 1 이상 109이하의 자연수
...
def fill(grid):
    global n
    for i in range(n): # 세로는 n
        valids = []
        cnt = 0
        for j in range(3 * n): # 0 아닌 거 긁기
            if grid[i][j]: # 
                cnt += 1
                valids.append(grid[i][j])
        nones = 3 * n - cnt
...
def getInit(n):
    ...
    for _ in range(3 * n):
        ...
        G.append(L)
    G = list(map(list, zip(*[el[::-1] for el in G])))
    # print(*G, sep = '\n')
    return G
...
def dfs(y, x, color, grid, vis):
    ...
    for dy, dx in zip(Dy, Dx):
        ny, nx = y + dy, x + dx
        if bound(ny, nx) and grid[ny][nx] == color:
            grid[ny][nx] = 0
            vis[ny][nx] = True ## 이거 잘 먹히겠지?
            cnt += dfs(ny, nx, color, grid, vis)
    return cnt

def simulate(depth, score, grid):
    # print('*')
    # print(depth, score)
    # print(*grid, sep = '\n')
    # print('*')
    ...
    vis = [ [False] * col for _ in range(row) ] # dfs 중복해서 안 돌릴 용도. 각 케이스마다 격자는 새로 만들어야한다.
    for i in range(row):
        for j in range(col - row, col): # 맨 오른쪽 row개만 본다.
            # print(*vis, sep = '\n')
            # print('?')
            if not vis[i][j] and grid[i][j] : 
                ...
                vis[i][j] = True
                newGrid = doCopy(grid, row)
                newGrid[i][j] = 0

                num = dfs(i, j, grid[i][j], newGrid, vis) # 같은 숫자 개수
                rec = (b[1] - a[1] + 1) * (c[0] - a[0] + 1) # 넓이
                # print(grid[i][j], '이', num, rec)

                # print(*newGrid, sep = '\n')
                newGrid = fill(newGrid) # 안꼬이는지 확인하기
                # print(*newGrid, sep = '\n')
                # print()
                newscore  = score + num + rec
                # print(newscore,'?')
                simulate(depth - 1, newscore, newGrid)
...
simulate(3, 0, grid) # depth = 3, try all
...
.mtku { text-decoration: underline; text-underline-position: under; }</style><style type="text/css" media="screen">
    ...
    <body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable loading-overlay-showing" style="background-color: rgb(255, 255, 255);"><div id="MathJax_Message" style="display: none;"></div>
            ...
            <div class="info">
                ...
                <div>
                    ...
                    <div class="limit_time" data-idx="1258" data-end-time="2021-08-28 15:00:00.0">
                        ...
                        <span class="info-txt type01 option_item value">42분 15초 </span>
                    </div>
        ...
        <div class="ide-body">
            <div class="ide-head">
                <h2 class="tit">Garage game</h2>
                
                
            </div>
            <div class="ide-content">
                <div class="ide-aside section pratice ui-resizable" id="SfproblemForm" style="width: 944px;">
                    <!-- 문제 내용 출력 -->
...
자율주행 기술의 발전과 함께 차량 내 인포테인먼트 기술 또한 많은 주목을 받고 있다. 최근 자동차 실내에는 디스플레이의 대형화를 비롯해 새로운 제어 기술이 빠르게 적용되고 있는데, 완전 자율주행 시대가 다가올 수록 이런 변화가 가속화될 전망이다. 개인 맞춤형 인포테인먼트 시스템 역시 핵심 기능 중 하나다. 자율주행 시대에 발맞춰 차량 내 디지털 시스템을 활용한 게임을 개발하고 있다.<br><br><img src="https://www.softeer.ai/upload/2021/08/20210826_141933551_97152.jpeg" style="width: 500px; height: 218px;"><br><br>
게임의 룰은 가로 세로 N칸의 차고(격자)가 있고, 각 차고에는 색깔이 있는 자동차가 하나씩 있다. 한 턴에 한 칸을 선택하며, 선택한 칸과 상하좌우 칸에 들어있는 자동차의 색이 같다면 모두 사라진다. 이때 획득할 수 있는 점수는 사라진 자동차의 개수와 사라지는 차고 칸을 모두 포함하는 가장 작은 직사각형의 넓이의 합이다.<br><br>
자동차들이 사라지고 나면 위에 있는 자동차들이 아래로 떨어져 빈 칸을 채운다. 위쪽에는 충분한 자동차들이 더 있어서 위에서 추가적으로 떨어지며 모든 칸을 채운다. 위에서 어떤 자동차들이 떨어질지는 입력으로 주어진다.<br><br>
위와 같은 게임을 3차례 반복 했을 때, 주어진 조건에서 얻을 수 있는 가장 큰 점수를 계산하라.
</div><p><br></p><div class="v-tit">입력형식</div><div class="v-txt">
입력으로는 차고 격자 칸의 가로, 세로 길이인 N이 첫 줄에 주어진다(1 ≤ N ≤ 15).<br>
다음 3N개의 줄에 N개의 자연수가 주어진다.<br>
각 자연수는 차고 칸에 있는 자동차의 색상 번호이다. 색상 번호는 1 이상 10<sup>9</sup>이하의 자연수이다.
...
3 3<br>
4 4<br>
1 2<br></div><p><br></p><div class="v-tit">출력예제1</div><div class="v-txt v-example">
15
</div><p><br></p><div class="v-tit">입력예제2</div><div class="v-txt v-example">
3<br>
8 5 1<br>
9 6 1<br>
10 7 1<br>
...
(미 입력 실행 시, 예제 자동 실행)"></textarea>
        ...
        <input type="hidden" id="data-psProblemId" value="540">
        <input type="hidden" id="data-groupId" value="G00001">
        <input type="hidden" id="data-practiceId" value="SW_PRBL_728">
